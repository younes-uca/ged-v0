...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const onMultiSelectChangeDocumentPartageGroupes = (e, field) => {
        if (e && e.value && Array.isArray(e.value)) {
            ...
            setdocumentPartageGroupes?cap_first(prevState => ({ ...prevState, [field]: selectedValues, }));
        }
    };
    ...
    const onMultiSelectChangeDocumentPartageUtilisateurs = (e, field) => {
        if (e && e.value && Array.isArray(e.value)) {
            const selectedValues = e.value.map(option => option && option.value);
            setdocumentPartageUtilisateurs?cap_first(prevState => ({ ...prevState, [field]: selectedValues, }));
        }
    };
    ...
    const onMultiSelectChangeDocumentManagementGroupes = (e, field) => {
        if (e && e.value && Array.isArray(e.value)) {
            const selectedValues = e.value.map(option => option && option.value);
            setdocumentManagementGroupes?cap_first(prevState => ({ ...prevState, [field]: selectedValues, }));
        }
    };
    ...
    const onMultiSelectChangeDocumentManagementUtilisateurs = (e, field) => {
        if (e && e.value && Array.isArray(e.value)) {
            const selectedValues = e.value.map(option => option && option.value);
            setdocumentManagementUtilisateurs?cap_first(prevState => ({ ...prevState, [field]: selectedValues, }));
        }
    };
    ...
    const saveItem = async () => {
        ...
        try {
            if (_item.id) {
                await DocumentService.update(_item).then((response) => {
                    console.log(response.data);
                    update(response.data);
                    onClose();
                });
                MessageService.showToast(showToast, { severity: 'success', summary: 'Successful', detail: 'Document Updated', life: 3000 });
            }
        } catch (error) {
            console.log(error);
            MessageService.showToast(showToast, { severity: 'Error', summary: 'Error', detail: 'Failed to save document', life: 3000 });
        }
    };
