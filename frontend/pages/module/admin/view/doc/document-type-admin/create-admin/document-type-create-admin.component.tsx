...
const Create = ({visible, onClose, add, showToast, list}) => {
    ...
    const saveItem = async () => {
        ...
        let _item = {...item};
        if (!_item.id) {
            const response = await DocumentTypeService.save(_item);
			_item.id = response.data.id;
            _items.push(_item);
            add(_item);
            MessageService.showToast(showToast, { severity: 'success', summary: 'Successful', detail: 'DocumentType Created', life: 3000 });
        }
        setItems(_items);
        onClose();
        setItem(emptyItem);
