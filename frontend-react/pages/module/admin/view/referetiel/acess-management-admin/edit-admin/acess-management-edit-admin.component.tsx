...
const Edit = ({visible, onClose, showToast, selectedItem, update}) => {
    ...
    const saveItem = async () => {
        ...
        try {
            if (_item.id) {
                await AcessManagementService.update(_item).then((response) => {
                    console.log(response.data);
                    update(response.data);
                    onClose();
                });
                MessageService.showToast(showToast, { severity: 'success', summary: 'Successful', detail: 'AcessManagement Updated', life: 3000 });
            }
        } catch (error) {
            console.log(error);
            MessageService.showToast(showToast, { severity: 'Error', summary: 'Error', detail: 'Failed to save acessManagement', life: 3000 });
        }
    };
